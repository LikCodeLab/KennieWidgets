...
package com.kennie.views.statemanager.state2
...
import com.kennie.views.statemanager.state2.Status.*
...
class StateLayout @JvmOverloads constructor(
    ...
    fun showLoading(tag: Any? = null, silent: Boolean = false, refresh: Boolean = true) {
        ...
        if (status == Status.LOADING) {
            onLoading?.invoke(getStatusView(Status.LOADING), tag)
            return
        }
        show(Status.LOADING, tag)
    }
